# 通知模块说明文档

> 本文档描述邻里共享平台的通知/消息模块设计与实现。

---

## 一、模块概述

通知模块用于处理平台内的各类消息通知，包括借阅申请通知、审批结果通知、归还提醒、超期提醒等。模块采用**定时任务 + 消息推送**的方式实现自动化提醒功能。

### 核心功能

| 功能 | 触发方式 | 说明 |
|-----|---------|------|
| 借阅申请通知 | 实时 | 用户发起借阅申请时，通知物品所有者 |
| 审批结果通知 | 实时 | 物品所有者审批后，通知申请人 |
| 归还申请通知 | 实时 | 借入者申请归还时，通知物品所有者 |
| 到期提醒 | 定时 | 每天早上 9 点，提醒明天到期的借阅 |
| 超期提醒 | 定时 | 每天早上 9 点，提醒已超期的借阅（隔天提醒） |

---

## 二、消息类型

```mermaid
mindmap
  root((消息类型))
    系统消息
      SYSTEM 系统通知
    借阅流程
      BORROW_APPLY 借阅申请
      BORROW_APPROVED 借阅已同意
      BORROW_REJECTED 借阅已拒绝
    归还流程
      RETURN_DUE 到期提醒
      RETURN_OVERDUE 超期提醒
      RETURN_CONFIRM 归还确认
```

### 消息类型详解

| 类型 | 枚举值 | 接收者 | 触发场景 |
|-----|--------|-------|---------|
| 系统通知 | `SYSTEM` | 全体/指定用户 | 系统公告 |
| 借阅申请 | `BORROW_APPLY` | 物品所有者 | 有人申请借用物品 |
| 借阅已同意 | `BORROW_APPROVED` | 申请人 | 借阅申请被通过 |
| 借阅已拒绝 | `BORROW_REJECTED` | 申请人 | 借阅申请被拒绝 |
| 到期提醒 | `RETURN_DUE` | 借入者 | 物品明天到期 |
| 超期提醒 | `RETURN_OVERDUE` | 借入者 | 物品已超期 |
| 归还确认 | `RETURN_CONFIRM` | 物品所有者 | 借入者申请归还 |

---

## 三、整体流程

```mermaid
flowchart TB
    subgraph 用户操作
        A[用户发起借阅申请] --> B[创建借阅记录]
        B --> C[发送 BORROW_APPLY 消息]
        
        D[物品所有者审批] --> E{审批结果}
        E -->|同意| F[发送 BORROW_APPROVED 消息]
        E -->|拒绝| G[发送 BORROW_REJECTED 消息]
        
        H[借入者申请归还] --> I[发送 RETURN_CONFIRM 消息]
    end
    
    subgraph 定时任务
        J[每天 9:00 执行] --> K{检查借阅状态}
        K --> L[明天到期?]
        L -->|是| M[发送 RETURN_DUE 消息]
        K --> N[已超期?]
        N -->|是| O[更新状态为 OVERDUE]
        O --> P[发送 RETURN_OVERDUE 消息]
    end
    
    subgraph 消息中心
        Q[用户查看消息] --> R[标记已读]
        S[全部已读] --> T[批量更新]
    end
```

---

## 四、借阅流程与消息触发

```mermaid
sequenceDiagram
    participant B as 借入者
    participant S as 后端服务
    participant L as 借出者
    participant M as 消息表
    participant T as 定时任务

    Note over B,T: 借阅申请阶段
    B->>S: 发起借阅申请
    S->>M: 写入 BORROW_APPLY 消息
    S-->>L: 推送通知（借阅申请）
    
    Note over B,T: 审批阶段
    L->>S: 审批（同意/拒绝）
    alt 同意
        S->>M: 写入 BORROW_APPROVED 消息
        S-->>B: 推送通知（申请已通过）
    else 拒绝
        S->>M: 写入 BORROW_REJECTED 消息
        S-->>B: 推送通知（申请已拒绝）
    end
    
    Note over B,T: 借用中 - 定时提醒
    T->>S: 每天 9:00 执行
    S->>S: 查询明天到期的借阅
    S->>M: 写入 RETURN_DUE 消息
    S-->>B: 推送通知（即将到期）
    
    S->>S: 查询已超期的借阅
    S->>S: 更新状态为 OVERDUE
    S->>M: 写入 RETURN_OVERDUE 消息
    S-->>B: 推送通知（已超期）
    
    Note over B,T: 归还阶段
    B->>S: 申请归还
    S->>M: 写入 RETURN_CONFIRM 消息
    S-->>L: 推送通知（归还确认）
    L->>S: 确认归还
    S->>S: 更新状态为 RETURNED
```

---

## 五、定时任务流程

```mermaid
flowchart TD
    A[定时任务启动<br/>每天 9:00] --> B[查询借阅记录]
    
    B --> C{到期提醒}
    C --> D[查询 status=ACTIVE<br/>且 end_date=明天]
    D --> E{今天是否已提醒?}
    E -->|否| F[发送 RETURN_DUE 消息]
    E -->|是| G[跳过]
    
    B --> H{超期提醒}
    H --> I[查询 status=ACTIVE/OVERDUE<br/>且 end_date < 今天<br/>且 last_remind_at < 2天前]
    I --> J{状态是否为 ACTIVE?}
    J -->|是| K[更新状态为 OVERDUE]
    J -->|否| L[保持 OVERDUE]
    K --> M[发送 RETURN_OVERDUE 消息]
    L --> M
    M --> N[更新 remind_count 和 last_remind_at]
    
    F --> O[任务完成]
    G --> O
    N --> O
```

### 定时任务配置

| 配置项 | 值 | 说明 |
|-------|-----|------|
| 执行时间 | `0 0 9 * * ?` | 每天早上 9 点 |
| 到期提前天数 | 1 天 | 提前 1 天提醒 |
| 超期提醒间隔 | 2 天 | 隔天提醒一次 |
| 技术方案 | Spring `@Scheduled` | 无需额外依赖 |

---

## 六、数据库设计

### messages 表结构

```mermaid
erDiagram
    MESSAGES {
        bigint id PK "消息ID"
        bigint from_user_id FK "发送者ID（可为空）"
        bigint to_user_id FK "接收者ID"
        varchar type "消息类型"
        varchar title "标题"
        text content "内容"
        bigint related_id "关联ID（借阅ID等）"
        boolean is_read "是否已读"
        timestamp created_at "创建时间"
    }
    
    USERS ||--o{ MESSAGES : "接收"
    USERS ||--o{ MESSAGES : "发送"
    BORROWS ||--o{ MESSAGES : "关联"
```

### 字段说明

| 字段 | 类型 | 说明 |
|-----|------|------|
| `id` | BIGINT | 主键，自增 |
| `from_user_id` | BIGINT | 发送者ID，系统消息为 NULL |
| `to_user_id` | BIGINT | 接收者ID，必填 |
| `type` | VARCHAR(20) | 消息类型枚举值 |
| `title` | VARCHAR(200) | 消息标题 |
| `content` | TEXT | 消息内容 |
| `related_id` | BIGINT | 关联业务ID（如借阅ID） |
| `is_read` | BOOLEAN | 是否已读，默认 FALSE |
| `created_at` | TIMESTAMP | 创建时间 |

### borrows 表相关字段

| 字段 | 类型 | 说明 |
|-----|------|------|
| `remind_count` | INT | 提醒次数 |
| `last_remind_at` | TIMESTAMP | 最后提醒时间 |

---

## 七、API 接口

### 消息相关接口

```mermaid
classDiagram
    class MessageController {
        +GET /api/messages 获取消息列表
        +GET /api/messages/unread 获取未读消息
        +GET /api/messages/unread-count 获取未读数量
        +POST /api/messages/:id/read 标记单条已读
        +POST /api/messages/read-all 全部标记已读
    }
```

| 接口 | 方法 | 说明 |
|-----|------|------|
| `/api/messages` | GET | 分页获取消息列表 |
| `/api/messages/unread` | GET | 分页获取未读消息 |
| `/api/messages/unread-count` | GET | 获取未读消息数量 |
| `/api/messages/{id}/read` | POST | 标记单条消息已读 |
| `/api/messages/read-all` | POST | 全部标记已读 |

### 请求/响应示例

**获取消息列表**
```http
GET /api/messages?page=0&size=10
Authorization: Bearer {token}

Response:
{
  "code": 0,
  "message": "OK",
  "data": {
    "content": [
      {
        "id": 1,
        "type": "RETURN_DUE",
        "typeDesc": "归还提醒",
        "title": "物品即将到期",
        "content": "您借用的「博世专业级电钻」将于明天到期，请及时归还",
        "relatedId": 123,
        "isRead": false,
        "createdAt": "2026-04-06T09:00:00"
      }
    ],
    "totalElements": 5,
    "totalPages": 1,
    "number": 0,
    "size": 10
  }
}
```

**获取未读数量**
```http
GET /api/messages/unread-count
Authorization: Bearer {token}

Response:
{
  "code": 0,
  "message": "OK",
  "data": {
    "count": 3
  }
}
```

---

## 八、前端集成

### 消息入口

导航栏右上角添加消息铃铛图标，显示未读消息数量红点：

```
[ 邻里共享. ]  [发现物品] [社区论坛] [发布闲置]    [用户名] [🔔³] [我的²] [退出]
```

### 页面路由

| 路由 | 页面 | 说明 |
|-----|------|------|
| `/messages` | MessageView.vue | 消息中心页面 |

### 页面功能

- 全部消息 / 未读消息 切换
- 消息列表展示（图标、类型、标题、内容、时间）
- 点击消息标记已读
- 一键全部已读
- 分页加载

---

## 九、关键代码位置

### 后端

| 模块 | 路径 |
|-----|------|
| 消息类型枚举 | `backend/demo/src/main/java/com/neighbor/enums/MessageType.java` |
| 消息实体 | `backend/demo/src/main/java/com/neighbor/entity/Message.java` |
| 消息 DTO | `backend/demo/src/main/java/com/neighbor/dto/MessageDTO.java` |
| 消息 Repository | `backend/demo/src/main/java/com/neighbor/repository/MessageRepository.java` |
| 消息 Service | `backend/demo/src/main/java/com/neighbor/service/MessageService.java` |
| 消息 Controller | `backend/demo/src/main/java/com/neighbor/controller/api/MessageController.java` |
| 定时任务 | `backend/demo/src/main/java/com/neighbor/scheduler/ReminderScheduler.java` |

### 前端

| 模块 | 路径 |
|-----|------|
| 消息 API | `frontend/src/api/message.ts` |
| 消息中心页面 | `frontend/src/views/MessageView.vue` |
| 导航栏组件 | `frontend/src/components/MainNav.vue` |
| 路由配置 | `frontend/src/router/index.ts` |

---

## 十、WebSocket 实时推送

### 概述

模块已升级为 **WebSocket 实时推送**模式，消息产生后立即推送给在线用户，无需轮询。

### WebSocket 架构

```mermaid
flowchart LR
    subgraph 前端
        A[用户登录] --> B[建立 WebSocket 连接]
        B --> C[监听消息]
        C --> D[更新 UI]
    end
    
    subgraph 后端
        E[消息产生] --> F[保存数据库]
        F --> G[WebSocket 推送]
    end
    
    G -->|实时推送| C
```

### 连接流程

```mermaid
sequenceDiagram
    participant F as 前端
    participant W as WebSocket
    participant B as 后端
    participant M as 消息服务

    F->>W: 登录后建立连接<br/>ws://host/ws/message?token=xxx
    W->>B: 握手请求
    B->>B: 验证 JWT Token
    B-->>W: 握手成功
    W-->>F: 连接建立
    
    Note over F,M: 消息推送
    M->>B: 发送消息
    B->>B: 保存数据库
    B->>W: 推送给用户
    W-->>F: 收到消息
    F->>F: 更新未读数量
    
    Note over F,M: 心跳保活
    F->>W: PING (每30秒)
    W-->>F: PONG
    
    Note over F,M: 断线重连
    W--xF: 连接断开
    F->>F: 等待 3 秒
    F->>W: 重新连接
```

### WebSocket 端点

| 端点 | 说明 |
|-----|------|
| `ws://localhost:8080/ws/message?token={jwt}` | WebSocket 连接地址 |

### 消息格式

**服务端推送消息格式**：
```json
{
  "type": "NEW_MESSAGE",
  "data": {
    "id": 1,
    "type": "BORROW_APPLY",
    "typeDesc": "借阅申请",
    "title": "新的借阅申请",
    "content": "张三 申请借用您的物品「博世专业级电钻」",
    "relatedId": 123,
    "isRead": false,
    "createdAt": "2026-04-06T10:30:00"
  }
}
```

**未读数量更新**：
```json
{
  "type": "UNREAD_COUNT",
  "data": {
    "count": 5
  }
}
```

### 事件-推送矩阵

每个业务事件发生时，推送给所有相关方：

| 业务事件 | 推送给谁 | 推送类型 | 推送内容 |
|---------|---------|---------|---------|
| 借阅申请 | 物品所有者 | `NEW_BORROW_APPLY` | 消息 + 角标+1 |
| 审批通过 | 借用者 | `REQUEST_APPROVED` | 申请已通过 |
| 审批通过 | 物品所有者 | `ITEM_STATUS_CHANGED` | 物品状态变更 + 角标-1 |
| 审批拒绝 | 借用者 | `REQUEST_REJECTED` | 申请已拒绝 |
| 审批拒绝 | 物品所有者 | `ITEM_STATUS_CHANGED` | 角标-1 |
| 归还申请 | 物品所有者 | `RETURN_REQUESTED` | 归还请求 + 角标+1 |
| 确认归还 | 借用者 | `BORROW_RETURNED` | 归还已完成 |
| 确认归还 | 物品所有者 | `RETURN_CONFIRMED` | 物品状态变回可用 + 角标-1 |

**推送消息示例**：
```json
{
  "type": "NEW_BORROW_APPLY",
  "data": {
    "message": "您有新的借阅申请",
    "itemId": 123,
    "pendingCount": 3
  }
}
```

```json
{
  "type": "ITEM_STATUS_CHANGED",
  "data": {
    "message": "物品状态已变更",
    "itemId": 123,
    "itemStatus": "BORROWED",
    "pendingCount": 2
  }
}
```

### 前端集成

```typescript
import { wsManager } from '../utils/websocket'

// 登录后建立连接
wsManager.connect(token)

// 统一消息处理
const handleWebSocketMessage = (message: any) => {
  const { type, data } = message
  
  switch (type) {
    case 'NEW_MESSAGE':
      unreadCount.value++
      break
    case 'NEW_BORROW_APPLY':
      pendingCount.value = data.pendingCount
      break
    case 'ITEM_STATUS_CHANGED':
      updateItemStatus(data.itemId, data.itemStatus)
      pendingCount.value = data.pendingCount
      break
    case 'RETURN_REQUESTED':
      returnRequestedCount.value = data.pendingCount
      break
    case 'RETURN_CONFIRMED':
    case 'BORROW_RETURNED':
      refreshData()
      break
  }
}

// 监听所有消息类型
wsManager.on('NEW_MESSAGE', handleWebSocketMessage)
wsManager.on('NEW_BORROW_APPLY', handleWebSocketMessage)
wsManager.on('ITEM_STATUS_CHANGED', handleWebSocketMessage)
wsManager.on('RETURN_REQUESTED', handleWebSocketMessage)
wsManager.on('RETURN_CONFIRMED', handleWebSocketMessage)
wsManager.on('REQUEST_APPROVED', handleWebSocketMessage)
wsManager.on('REQUEST_REJECTED', handleWebSocketMessage)
wsManager.on('BORROW_RETURNED', handleWebSocketMessage)

// 退出时断开连接
wsManager.disconnect()
```

### 关键代码位置

| 模块 | 路径 |
|-----|------|
| WebSocket 配置 | `backend/demo/src/main/java/com/neighbor/websocket/WebSocketConfig.java` |
| 认证拦截器 | `backend/demo/src/main/java/com/neighbor/websocket/WebSocketAuthInterceptor.java` |
| 消息处理器 | `backend/demo/src/main/java/com/neighbor/websocket/WebSocketMessageHandler.java` |
| 推送服务 | `backend/demo/src/main/java/com/neighbor/websocket/WebSocketPushService.java` |
| 推送通知 DTO | `backend/demo/src/main/java/com/neighbor/dto/PushNotification.java` |
| 前端连接管理 | `frontend/src/utils/websocket.ts` |

### 特性

| 特性 | 说明 |
|-----|------|
| JWT 认证 | 连接时通过 URL 参数传递 token |
| 心跳保活 | 每 30 秒发送 PING 保持连接 |
| 断线重连 | 自动重连，最多 5 次 |
| 用户管理 | 维护 userId ↔ Session 映射 |

---

## 十一、扩展方向

1. **用户私信** - 基于 WebSocket 实现用户间实时聊天
2. **消息模板** - 支持自定义消息模板
3. **消息订阅** - 用户可选择订阅/取消订阅某类消息
4. **短信/邮件通知** - 重要消息支持短信或邮件通知
5. **消息聚合** - 相同类型消息聚合展示，避免刷屏

---

## 十二、注意事项

1. **定时任务单机模式**：当前使用 Spring `@Scheduled`，仅适用于单机部署。如需集群部署，需迁移至 Quartz 或 XXL-JOB

2. **WebSocket 单机模式**：当前 WebSocket 连接管理为内存模式，仅适用于单机部署。如需集群部署，需引入 Redis Pub/Sub

3. **消息去重**：定时任务中已实现当天不重复发送到期提醒的逻辑

4. **超期状态**：定时任务会自动将超期借阅的状态从 `ACTIVE` 更新为 `OVERDUE`

5. **消息清理**：建议定期清理已读超过 30 天的消息，避免数据膨胀
